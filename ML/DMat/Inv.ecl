...
EXPORT Inv(IMatrix_Map map_a, DATASET(Part) A, BOOLEAN findLeft=FALSE) := FUNCTION
  map_at := PBblas_v0.Matrix_Map(map_a.matrix_cols, map_a.matrix_rows,
                             map_a.part_cols(1), map_a.part_rows(1));
  AT := PBblas_v0.PB_dtran(map_a, map_at, 1.0, A);
  ...
  map_c  := PBblas_v0.Matrix_Map(C_rows, C_cols, C_prow, C_pcol);
  C := IF(findLeft,
          PBblas_v0.PB_dgemm(FALSE, TRUE, 1.0, map_a, A, map_at, A, map_c),
          PBblas_v0.PB_dgemm(TRUE, FALSE, 1.0, map_at, A, map_a, A, map_c));
  F := PBblas_v0.PB_dpotrf(IF(findLeft, Triangle.Upper, Triangle.Lower), map_c, C);
  sideSw := IF(findLeft, Side.xA, Side.Ax);
  S := PBblas_v0.PB_dtrsm(sideSw, IF(findLeft, Triangle.Upper, Triangle.Lower), FALSE,
                       Diagonal.NotUnitTri, 1.0, map_c, F, map_at, AT);
  T := PBblas_v0.PB_dtrsm(sideSw, IF(findLeft, Triangle.Lower, Triangle.Upper), TRUE,
