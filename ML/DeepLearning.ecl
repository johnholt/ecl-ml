...
//prows, pcols, Maxrows, Maxcols for the PBblas_v0 partitioning:
...
EXPORT Sparse_Autoencoder (UNSIGNED4 prows=0, UNSIGNED4 pcols=0, UNSIGNED4 Maxrows=0, UNSIGNED4 Maxcols=0) := MODULE
  ...
  SHARED SA(DATASET(Types.NumericField) X, DATASET(Mat.Types.MUElement) IntW, DATASET(Mat.Types.MUElement) Intb, REAL8 BETA=0.1, REAL8 sparsityParam=0.1 , REAL8 LAMBDA=0.001, REAL8 ALPHA=0.1, UNSIGNED2 MaxIter=100) := MODULE
   ...
   //Map for Matrix d.
    ...
    //functions used
    PBblas_v0.Types.value_t sp_reci(PBblas_v0.Types.value_t v,PBblas_v0.Types.dimension_t r,PBblas_v0.Types.dimension_t c) := sparsityParam_/v;
    PBblas_v0.Types.value_t sp_1_reci(PBblas_v0.Types.value_t v,PBblas_v0.Types.dimension_t r,PBblas_v0.Types.dimension_t c) := sparsityParam_1/(1-v);
    //sparsity_delta=((-sparsityParam./rhohat)+((1-sparsityParam)./(1.-rhohat)));
    PBblas_v0.Types.value_t sp_delta(PBblas_v0.Types.value_t v,PBblas_v0.Types.dimension_t r,PBblas_v0.Types.dimension_t c) := (sparsityParam_/v)+(sparsityParam_1/(1-v));
    ...
    FF2(DATASET(Layout_Part) w1, DATASET(Layout_Part) b1v):= FUNCTION
      //b1m = repmat(b1v,1,m)
      b1m := PBblas_v0.PB_dgemm(FALSE, TRUE, 1.0,b1vecmap, b1v, Ones_VecMap, Ones_Vecdist, b1map);
      //z2 = w1*X+b1;
      z2 := PBblas_v0.PB_dgemm(FALSE, FALSE, 1.0,w1map, w1, dmap, ddist, b1map, b1m, 1.0);
      //a2 = sigmoid (z2);
    ...
    FF3(DATASET(Layout_Part) w2,DATASET(Layout_Part) b2v, DATASET(Layout_Part) a2 ):= FUNCTION
      //b2m = repmat(b2v,1,m)
      b2m := PBblas_v0.PB_dgemm(FALSE, TRUE, 1.0,b2vecmap, b2v, Ones_VecMap, Ones_Vecdist, b2map);
      //z3 = w2*a2+b2;
      //z3 := PBblas_v0.PB_dgemm(FALSE, FALSE,1.0,w2map, w2, a2map, a2, b2map,b2m, 1.0);
      z3_tmp := PBblas_v0.PB_dgemm(FALSE, FALSE,1.0,w2map, w2, a2map, a2, b2map);
      z3 := PBblas_v0.PB_daxpy(1.0, z3_tmp, b2m);
      //a3 = sigmoid (z3)
      a3 := PBblas_v0.Apply2Elements(b2map, z3, sigmoid);
      RETURN a3;
    END;//END FF3
    //DELTA3 returns d3
    DELTA3 (DATASET(Layout_Part) a3 ) := FUNCTION
      ...
      //y=X;
      //d3=-(y-a3).*(a3.*(1-a3));
      siggrad_a3 := PBblas_v0.Apply2Elements(a3map, a3, siggrad);
      a3_y := PBblas_v0.PB_daxpy(-1, ddist, a3);
      d3 := PBblas_v0.HadamardProduct(a3map, a3_y, siggrad_a3);
      RETURN d3 ;
    END;//END DELTA3
    //DELTA2 retunrs d2
    DELTA2 (DATASET(Layout_Part) w2, DATASET(Layout_Part) a2, DATASET(Layout_Part) d3) := FUNCTION
      ...
      rhohat := PBblas_v0.PB_dgemm(FALSE, FALSE, m_1,  a2map, a2, Ones_VecMap, Ones_Vecdist, Hiddmap);
      sparsity_delta := PBblas_v0.Apply2Elements(Hiddmap, rhohat, sp_delta);
      siggrad_a2 := PBblas_v0.Apply2Elements(a2map, a2, siggrad);
      repmat_sparsity_delta := PBblas_v0.PB_dgemm(FALSE, TRUE, 1.0,  Hiddmap, sparsity_delta, Ones_VecMap, Ones_Vecdist, a2map);
      //d2_firstterm = (W2'*d3)+beta*repmat(sparsity_delta,1,m);
      d2_firstterm := PBblas_v0.PB_dgemm(TRUE, FALSE, 1.0, w2map, w2, a3map, d3, a2map, repmat_sparsity_delta, BETA);
      d2 := PBblas_v0.HadamardProduct(a2map, d2_firstterm, siggrad_a2);
      RETURN d2 ;
    END;
    //WeightGrad1 returns gradient for w1
    WeightGrad1 (DATASET(Layout_Part) w1, DATASET(Layout_Part) d2) := FUNCTION
      w1_g := PBblas_v0.PB_dgemm(FALSE, TRUE, m_1, a2map, d2, dmap, ddist, w1map, w1 ,LAMBDA );
      RETURN w1_g;
    END;
    ...
    WeightGrad2 (DATASET(Layout_Part) w2, DATASET(Layout_Part) d3, DATASET(Layout_Part) a2) := FUNCTION
      w2_g := PBblas_v0.PB_dgemm(FALSE, TRUE, m_1, a3map, d3, a2map, a2, w2map, w2 ,LAMBDA );
      RETURN w2_g;
    END;
    //BiasGrad1 calculates the bias gradients for b1
    BiasGrad1 (DATASET(Layout_Part) d2) := FUNCTION
      b1_g := PBblas_v0.PB_dgemm(FALSE, FALSE, m_1,  a2map, d2, Ones_VecMap, Ones_Vecdist, b1vecmap);
      RETURN b1_g;
    END;
    //BiasGrad2 calculates the bias gradients for b2
    BiasGrad2 (DATASET(Layout_Part) d3) := FUNCTION
      b2_g := PBblas_v0.PB_dgemm(FALSE, FALSE, m_1,  a3map, d3, Ones_VecMap, Ones_Vecdist, b2vecmap);
      RETURN b2_g;
    END;
    GradDesUpdate (DATASET(Layout_Part) tobeUpdated, DATASET(Layout_Part) GradDesTerm):= FUNCTION
      tmp_updated := PBblas_v0.PB_daxpy(-1, PBblas_v0.PB_dscal(ALPHA, GradDesTerm), tobeUpdated);
      RETURN tmp_updated;
    END;
    GradDesLoop (DATASET(Layout_Part) w1in, DATASET(Layout_Part) w2in, DATASET(Layout_Part) bvec1in, DATASET(Layout_Part) bvec2in):= FUNCTION
      w1inno := PBblas_v0.MU.TO(w1in, 1);
      w2inno := PBblas_v0.MU.TO(w2in, 2);
      bvec1inno := PBblas_v0.MU.TO(bvec1in, 3);
      bvec2inno := PBblas_v0.MU.TO(bvec2in, 4);
      prm := w1inno + w2inno + bvec1inno + bvec2inno;
      GradDesLoop_Step (DATASET(PBblas_v0.Types.MUElement) Inputprm) := FUNCTION
        w1m := PBblas_v0.MU.FROM (Inputprm, 1);
        w2m := PBblas_v0.MU.FROM (Inputprm, 2);
        b1v := PBblas_v0.MU.FROM (Inputprm, 3);
        b2v := PBblas_v0.MU.FROM (Inputprm, 4);
        a2 := FF2 (w1m, b1v);
        a3 := FF3 (w2m, b2v, a2);
        d3 := DELTA3 (a3);
        d2 := DELTA2 (w2m, a2, d3);
        ...
        w1uno := PBblas_v0.MU.TO (w1u, 1);
        w2uno := PBblas_v0.MU.TO (w2u, 2);
        b1uno := PBblas_v0.MU.TO (b1u, 3);
        b2uno := PBblas_v0.MU.TO (b2u, 4);
        // prmu := IF (coun=1, w1uno + w2uno + b1uno + b2uno,PBblas_v0.MU.TO (a2, 2)+PBblas_v0.MU.TO (w2m, 1)+PBblas_v0.MU.TO (d3, 3)+PBblas_v0.MU.TO (d2, 4));
      ...
      RETURN finalprm;
    END;//END GradDesLoop
    SAprm := GradDesLoop (w1dist, w2dist, b1vecdist, b2vecdist);// SAprm is in PBblas_v0.Types.MUElement format convert it to
    //numericfield format
    SAprm1 := PBblas_v0.MU.From (SAprm,1);
    SAprm2 := PBblas_v0.MU.From (SAprm,2);
    SAprm3 := PBblas_v0.MU.From (SAprm,3);
    SAprm4 := PBblas_v0.MU.From (SAprm,4);
