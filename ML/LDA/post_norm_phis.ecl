...
EXPORT Types.Topic_Values_DataSet
  ...
  while (consumed < lenT_phis) {
    ...
    const LDAOnlyValue* in_phis = (LDAOnlyValue*) (((uint8_t*)t_phis) + consumed + fx_size);
    LDATopicValues* rslt_t_phis = (LDATopicValues*) (((uint8_t*)__result) + consumed);
    LDAOnlyValue* rslt_phis = (LDAOnlyValue*) (((uint8_t*)__result) + fx_size + consumed);
    if (vs_size!=in_t_phis->sz_vs) {
      sprintf(msg_buffer,
              "Unexpected number of phi, %lu expected; found %u. ",
