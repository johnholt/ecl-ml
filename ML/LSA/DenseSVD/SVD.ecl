...
EXPORT SVD(IMatrix_map a_map, DATASET(Part) A) := MODULE
    ...
    EXPORT MGN() := FUNCTION
      ...
      AtA := PBblas_v0.PB_dgemm(TRUE, FALSE, 1.0, a_map, A, a_map, A, aTa_map);
      ...
      S := PBblas_v0.Apply2Elements(aTa_map, S1, Sqroot);
      InvS := PBblas_v0.Apply2Elements(aTa_map, S, Reciprocal);
      U := DMat.Mul(a_map, DMat.Mul(a_map, A, aTa_map, V, a_map), aTa_map, InvS, a_map);
      RETURN PBblas_v0.MU.To(U, mat_type.U) + PBblas_v0.MU.To(S, mat_type.S) + PBblas_v0.MU.To(V, mat_type.V);
    END;
    
    EXPORT MLN() := FUNCTION
      aT_map := DMat.Trans.TranMap(a_map);
      aaT_map := PBblas_v0.Matrix_Map(a_map.matrix_rows, aT_map.matrix_cols, a_map.part_rows(1), aT_map.part_cols(1));
      AAt := PBblas_v0.PB_dgemm(FALSE, TRUE, 1.0, a_map, A, a_map, A, aaT_map);
      ...
      S := PBblas_v0.Apply2Elements(aaT_map, S1, Sqroot);
      InvS := PBblas_v0.Apply2Elements(aaT_map, S, Reciprocal);
      V := DMat.Mul(aT_map, PBblas_v0.PB_dgemm(TRUE, FALSE, 1.0, a_map, A, aaT_map, U, aT_map), aaT_map, InvS, aT_map);
      RETURN PBblas_v0.MU.To(U, mat_type.U) + PBblas_v0.MU.To(S, mat_type.S) + PBblas_v0.MU.To(V, mat_type.V);
    END;
