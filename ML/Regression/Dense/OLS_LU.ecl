...
Unit      := PBblas_v0.Types.Diagonal.UnitTri;
...
:= MODULE(ML.Regression.Dense.OLS(X,Y))
  ...
  XtY_p := PBblas_v0.PB_dbvmm(TRUE, FALSE, 1.0, x2_map, x_part, y2_map, y_part,
                          b2_map);
  LU_p  := PBblas_v0.PB_dgetrf(z2_map, XtX_p);
  s1_p  := PBblas_v0.PB_dtrsm(Side.Ax, LowerTri, FALSE, Unit, 1.0,
                          z2_map, LU_p, b2_map, XtY_p);
  b_part:= PBblas_v0.PB_dtrsm(Side.Ax, UpperTri, FALSE, NotUnit, 1.0,
