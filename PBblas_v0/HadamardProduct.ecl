...
Cell := PBblas_v0.Types.Layout_Cell;
MakeR8Set := PBblas_v0.MakeR8Set;

//x_map - a map that must be the same for both X and Y datasets
//X - dataset in PBblas_v0 format
//Y - dataset in PBblas_v0 format

EXPORT DATASET(Part)
      HadamardProduct(IMatrix_Map x_map, DATASET(Part) X, DATASET(Part) Y) := FUNCTION
  Product(PBblas_v0.Types.value_t val1, PBblas_v0.Types.value_t val2) := val1 * val2;
  ...
  Part mulPart(Part xrec, Part yrec) := TRANSFORM
    ...
    SELF.mat_part := MAP(haveX AND haveY =>  SET(new_elems, v),
                       haveX  => MakeR8Set(xrec.part_rows, xrec.part_cols,
                                           xrec.first_row, xrec.first_col,
                                           DATASET([], Cell), 0, 0.0),
                       PBblas_v0.MakeR8Set(yrec.part_rows, yrec.part_cols,
                                           yrec.first_row, yrec.first_col,
                                           DATASET([], Cell), 0, 0.0));
    SELF := xrec;
