...
// First check maps for compatability.  Normalize for transpose operations.
  ...
  A := ASSERT(inA,
              ASSERT(c_row_blocks=1 AND c_col_blocks=1,
                    'Product ' + PBblas_v0.Constants.Not_Single_Block, FAIL),
              ASSERT(a_row_blocks=1, PBblas_v0.Constants.Not_Block_Vector, FAIL),
              ASSERT(a_matrix_cols=b_matrix_rows AND a_col_blocks=b_row_blocks,
                    'A-B ' + PBblas_v0.Constants.Dimension_Incompat, FAIL),
              ASSERT(a_matrix_rows=c_matrix_rows AND a_row_blocks=c_row_blocks,
                    'A-C ' + PBblas_v0.Constants.Dimension_Incompat, FAIL));
  B := ASSERT(inB,
              ASSERT(c_row_blocks=1 AND c_col_blocks=1,
                    'Product ' + PBblas_v0.Constants.Not_Single_Block, FAIL),
              ASSERT(b_col_blocks=1, PBblas_v0.Constants.Not_Block_Vector, FAIL),
              ASSERT(a_matrix_cols=b_matrix_rows AND a_col_blocks=b_row_blocks,
                    'A-B ' + PBblas_v0.Constants.Dimension_Incompat, FAIL),
              ASSERT(b_matrix_cols=c_matrix_cols AND b_col_blocks=c_col_blocks,
                    'B-C ' + PBblas_v0.Constants.Dimension_Incompat, FAIL));
