...
Distribution_Error := PBblas_v0.Constants.Distribution_Error;
Dimension_Incompat := PBblas_v0.Constants.Dimension_Incompat;
Not_Square         := PBblas_v0.Constants.Not_Square;
BaseTerm := 1;
RightTerm := 3;
LeftTerm := 2;
...
EXPORT PB_dtrsm(Side s, Triangle tri, BOOLEAN transposeA, Diagonal diag,
                value_t alpha,
                PBblas_v0.IMatrix_Map a_map, DATASET(Part) rawA,
                PBblas_v0.IMatrix_Map b_map, DATASET(Part) inB) := FUNCTION
  // Fix transpose of A if required.  This should not be required with more complex
  //conditions.
  inA := IF(transposeA, PBblas_v0.PB_dtran(a_map, a_map, 1.0, rawA), rawA);
  ...
  loopBody(DATASET(Part) parts, UNSIGNED4 loop_c) := FUNCTION
    ...
    updated := JOIN(inpSet,
                    ...
                    updatePart(ROWS(LEFT)), SORTED(t_node_id, t_part_id),
                    MOFN(1), LOCAL);
